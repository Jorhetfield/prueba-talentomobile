...
fun View.visible() =
    this.post { if (this.visibility != View.VISIBLE) this.visibility = View.VISIBLE }

fun View.invisible() =
    this.post { if (this.visibility != View.INVISIBLE) this.visibility = View.INVISIBLE }
...
fun View.slideBottom(visible: Boolean, animDuration : Long = 600) {
    val transition : Transition =  Slide(Gravity.BOTTOM)
    transition.apply {
        duration = animDuration
        addTarget(this@slideBottom)
    }
    TransitionManager.beginDelayedTransition(this@slideBottom.parent as ViewGroup, transition)
    this@slideBottom.visibility = when(visible) {
        true -> {
            View.VISIBLE
        }
        false -> {
...
fun View.slideBottom(animDuration : Long = 600) {
    val transition : Transition =  Slide(Gravity.BOTTOM)
    transition.apply {
        duration = animDuration
        addTarget(this@slideBottom)
    ...
    TransitionManager.beginDelayedTransition(this@slideBottom.parent as ViewGroup, transition)
    this@slideBottom.visibility = when(this@slideBottom.visibility) {
        View.GONE -> {
            View.VISIBLE
        }
        View.VISIBLE -> {
...
fun View.colorizeTint(
    from: Int = this@colorizeTint.solidColor,
    to: Int,
    duration: Long = 300,
    delay: Long = 0
) {
